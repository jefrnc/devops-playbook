...
class CommitTracker:
    ...
    def get_commits_with_deployments(
        ...
        for repo_name in self.config.get("repositories", []):
            try:
                repo = self.client.get_repo(
                    f"{self.config['organization']}/{repo_name}"
                )
                ...
                for commit in commits:
                    ...
                    if deployment_time:
                        lead_time = (
                            deployment_time - commit.commit.author.date
                        ).total_seconds() / 3600
        ...
        for project_id in self.config.get("projects", []):
            try:
                ...
                commits = project.commits.list(
                    since=start_date.isoformat(), until=end_date.isoformat(), all=True
                )

                for commit in commits:
                    # Find deployment for this commit
                    deployment_time = self._find_gitlab_deployment_time(
                        project, commit.id
                    )
    ...
    def _find_gitlab_deployment_time(
        self, project, commit_sha: str
    ) -> Optional[datetime]:
...
class LeadTimeCalculator:
    ...
    def calculate(self) -> Dict:
        ...
        # Group by repository
        by_repository = (
            df.groupby("repository")["lead_time_hours"]
            .agg(["mean", "count"])
            .to_dict("index")
        )
        ...
        percentiles = {
            ...
            "p75": (
                statistics.quantiles(lead_times, n=4)[2]
                if len(lead_times) > 3
                else max(lead_times)
            ),
            "p90": (
                statistics.quantiles(lead_times, n=10)[8]
                if len(lead_times) > 9
                else max(lead_times)
            ),
            "p95": (
                statistics.quantiles(lead_times, n=20)[18]
                if len(lead_times) > 19
                else max(lead_times)
            ),
        }
        ...
        performance_level = self._determine_performance_level(
            metrics["median_lead_time_hours"]
        )
...
class LeadTimeReporter:
    ...
    def generate_report(metrics: Dict):
        ...
        print(
            f"  Mean: {stats['mean_lead_time_hours']:.1f} hours ({stats['mean_lead_time_days']:.1f} days)"
        )
        print(
            f"  Median: {stats['median_lead_time_hours']:.1f} hours ({stats['median_lead_time_days']:.1f} days)"
        )
        ...
        if results.get("by_repository"):
            ...
            print(
                tabulate(
                    repo_data,
                    headers=["Repository", "Avg Hours", "Deployments"],
                    tablefmt="grid",
                )
            )
...
@click.option(
    "--output",
    type=click.Choice(["json", "csv", "report"]),
    default="report",
    help="Output format",
)
