...
class DeploymentSource(ABC):
    ...
    @abstractmethod
    def get_deployments(
        self, start_date: datetime, end_date: datetime, environments: List[str]
    ) -> List[Dict]:
...
class DeploymentFrequencyCalculator:
    ...
    def _calculate_trend(self, daily_counts: pd.Series) -> str:
        ...
        prev_week = (
            daily_counts[-14:-7].mean()
            if len(daily_counts) >= 14
            else daily_counts[:-7].mean()
        )
...
class MetricsExporter:
    ...
    def export_prometheus(metrics: Dict, push_gateway: Optional[str] = None):
        ...
        # Create gauges
        deployment_total = Gauge(
            "deployment_frequency_total",
            "Total deployments in period",
            registry=registry,
        )
        deployment_daily = Gauge(
            "deployment_frequency_daily_average",
        ...
        # By environment
        deployment_by_env = Gauge(
            "deployment_frequency_by_environment",
    ...
    def generate_report(metrics: Dict):
        ...
        if results["by_environment"]:
            print(f"\nDeployments by Environment:")
            env_data = [
                [env, count] for env, count in results["by_environment"].items()
            ]
        ...
        if results["by_service"]:
            ...
            service_data = sorted(
                results["by_service"].items(), key=lambda x: x[1], reverse=True
            )[:10]
