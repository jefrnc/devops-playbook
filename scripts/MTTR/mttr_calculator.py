...
class Incident:
    ...
    def calculate_mttr(self):
        ...
        if self.resolved_at and self.created_at:
            self.mttr_minutes = (self.resolved_at - self.created_at).total_seconds() / 60
            ...
            for inc in data['incidents']:
                ...
                if inc['status'] == 'resolved':
                    incident = Incident(
                        ...
                        resolved_at=parse(inc['resolved_at']) if inc.get('resolved_at') else None,
                        severity=self._map_severity(inc['urgency']),
                        service=inc['service']['summary'] if inc.get('service') else 'Unknown'
                    )
    ...
    def get_incidents(self, start_date: datetime, end_date: datetime) -> List[Incident]:
        ...
        for alert in data['data']:
            ...
            detail_response = requests.get(
                f"{self.base_url}/alerts/{alert['id']}",
                headers=self.headers
            )
...
class CloudWatchSource(IncidentSource):
        ...
        for alarm_name, events in alarm_events.items():
            ...
            while i < len(events):
                ...
                if event['HistorySummary'].startswith('Alarm updated from OK to ALARM'):
                    ...
                    j = i + 1
                    while j < len(events):
                        if events[j]['HistorySummary'].startswith('Alarm updated from ALARM to OK'):
    ...
    def _determine_severity(self, alarm_name: str) -> str:
        ...
        if any(keyword in alarm_name.lower() for keyword in ['critical', 'p1', 'outage']):
...
class MTTRCalculator:
    ...
    def _initialize_source(self) -> IncidentSource:
        ...
        if source_type == 'pagerduty':
            api_key = os.environ.get('PAGERDUTY_TOKEN') or self.config.get('pagerduty', {}).get('api_key')
            if not api_key:
                raise ValueError("PagerDuty API key not found")
            return PagerDutySource(api_key)
            
        elif source_type == 'opsgenie':
            api_key = os.environ.get('OPSGENIE_TOKEN') or self.config.get('opsgenie', {}).get('api_key')
            if not api_key:
    ...
    def calculate(self) -> Dict:
        ...
        if severity_filter:
            incidents = [i for i in incidents if any(sev in i.severity for sev in severity_filter)]
        ...
        metrics = {
            ...
            'std_dev_minutes': statistics.stdev(mttr_values) if len(mttr_values) > 1 else 0
        }
        ...
        by_severity = df.groupby('severity')['mttr_minutes'].agg(['mean', 'count']).to_dict('index')
        
        # Group by service
        by_service = df.groupby('service')['mttr_minutes'].agg(['mean', 'count']).to_dict('index')
        
        # Calculate percentiles
        percentiles = {
            'p50': statistics.quantiles(mttr_values, n=2)[0],
            'p75': statistics.quantiles(mttr_values, n=4)[2] if len(mttr_values) > 3 else max(mttr_values),
            'p90': statistics.quantiles(mttr_values, n=10)[8] if len(mttr_values) > 9 else max(mttr_values),
            'p95': statistics.quantiles(mttr_values, n=20)[18] if len(mttr_values) > 19 else max(mttr_values)
        }
        
        # Determine performance level
        performance_level = self._determine_performance_level(metrics['median_mttr_minutes'])
        ...
        longest_incidents = df.nlargest(5, 'mttr_minutes')[['id', 'title', 'mttr_minutes', 'severity']].to_dict('records')
...
class MTTRReporter:
    ...
    def generate_report(metrics: Dict):
        ...
        print(f"\nMTTR Statistics:")
        print(f"  Mean: {stats['mean_mttr_minutes']:.1f} minutes ({stats['mean_mttr_hours']:.1f} hours)")
        print(f"  Median: {stats['median_mttr_minutes']:.1f} minutes ({stats['median_mttr_hours']:.1f} hours)")
        ...
        if results.get('by_severity'):
            ...
            print(tabulate(severity_data, headers=['Severity', 'Avg Minutes', 'Count'], tablefmt='grid'))
        
        if results.get('by_service'):
            print(f"\nMTTR by Service (Top 10):")
            service_data = []
            sorted_services = sorted(results['by_service'].items(), 
                                   key=lambda x: x[1]['count'], reverse=True)[:10]
            for service, stats in sorted_services:
                service_data.append([service[:30], f"{stats['mean']:.1f}", stats['count']])
            print(tabulate(service_data, headers=['Service', 'Avg Minutes', 'Count'], tablefmt='grid'))
